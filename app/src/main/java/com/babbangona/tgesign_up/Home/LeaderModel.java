...
public class LeaderModel {
    ...
    @SuppressLint("StaticFieldLeak")
    public int getRegisteredMemberCountResult(Context context, String unique_member_id) {
        int result ;
        try {
            result = new getRegisteredMemberCount(context){
                @Override
                protected void onPostExecute(Integer s) {}
            }.execute(unique_member_id).get();
        } catch (ExecutionException | InterruptedException e) {
    ...
    @SuppressLint("StaticFieldLeak")
    public static abstract class getRegisteredMemberCount extends AsyncTask<String, Void, Integer> {
        Context mCtx;
        TFMDatabase tfmDatabase;

        getRegisteredMemberCount(Context context) {
            this.mCtx = context;
        }
        ...
        protected Integer doInBackground(String... strings) {
            try{
                Log.d("answer_member_count", Arrays.toString(strings));
                tfmDatabase = TFMDatabase.getInstance(mCtx);
                return tfmDatabase.getMembersTable().getRegisteredMemberCount(strings[0]);
            }catch (Exception e){
                e.printStackTrace();
                return 0;
            }
